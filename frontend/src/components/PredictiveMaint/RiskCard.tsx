...
function RiskGauge({ score }: { score: number }) {
  ...
  const color = score >= 70 ? "var(--crit)" : score >= 40 ? "var(--warn)" : "var(--ok)";
  return (
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <Wrench size={14} className="text-[var(--muted2)]" />
            <span className="text-sm font-semibold text-[var(--text-strong)]">설비 #{report.equipment_id}</span>
          ...
          {report.llm_summary && (
            <div className="text-xs text-[var(--muted2)] leading-relaxed whitespace-pre-wrap line-clamp-4">
              {report.llm_summary}
            </div>
          )}
...
export function PredictiveMaintPanel() {
  ...
  if (isLoading) return <div className="text-[var(--muted)] text-sm p-4">예지보전 데이터 로딩 중…</div>;
