# PNT Smart Factory Monitor — 프로젝트 이해 가이드
...
| LLM | **OpenAI gpt-5.4-nano** | 단일 경량 모델 — 포트폴리오 데모 특성상 API 비용 최소화 우선 |
...
- 주요 필드:
  - `openai_model_default = "gpt-5.4-nano"` — 비용 절감을 위해 단일 모델 사용
...
### `routing.py` — 모델 선택 (nano 단일)
포트폴리오 데모 특성상 API 비용 최소화가 우선이라 **gpt-5.4-nano 단일 모델**만 사용.
복잡도 기반 nano/mini 자동 라우팅을 구현했었으나 제거 (git history에 보존 — "비용 대비
가치 판단으로 기능을 덜어낸" 사례로 면접에서 설명 가능). 비용 통제 장치는 모델 외에도
토큰 상한(1024), 대화 이력 6턴 제한, 메시지 500자 제한, IP/세션 rate limit이 있음.
...
- "벡터DB 없이 **통계적 특징 벡터 + 유클리드 거리 기반 유사사례 검색**으로 RAG 유사 구조의 예지보전 리포트를 LLM(gpt-5.4-nano)으로 생성했습니다."
- "Pydantic AI 기반 에이전트가 **7개의 MES 도구(시계열 조회, 이상이력, 상관분석, 설비상태, 예지보전, 교대보고서, 테스트 주입)**를 함수 호출로 활용해 자연어 질의에 답합니다."
- "포트폴리오 데모 특성에 맞게 **경량 모델(nano) 단일 구성 + 토큰 상한 + rate limit**으로 API 비용을 통제했습니다."
